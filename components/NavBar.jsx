...
import {FaBars, FaHome, FaBook, FaUser, FaInstagram, FaGithub, FaDiscord} from 'react-icons/fa'
...
const NavLink = (props) => {
    ...
    return(
        <Link href={props.Href} passHref>
            <a className={(router.pathname === props.Href ?'active ': '') + 'flex justify-center items-center text-center px-2 py-1 tracking-[0.3rem] hover:tracking-[0.5rem] transition-all duration-300'}>
                {props.name}
            </a>
        </Link>
    )
    ...
    return(
        <Link href={props.linkPath} passHref>
            <a className={(router.pathname === props.linkPath ? "bg-[#8C77A0]" : "") + " flex items-center text-white px-2 rounded-xl w-[80%] h-[6vh] m-2 hover:bg-[#8C77A0] transition-all duration-300"}>
                <props.iconName className='text-[1.5rem]'/>
...
const SubNav = () => {
    return(
        <div className="lg:flex w-full hidden">
            <div className="flex w-[40%] justify-around xl:text-[1.2rem] font-light mx-auto">
                <NavLink
                    name='Home'
                    Href='/'
                />
            ...
            <div className="flex w-[25%] justify-center">
                <Link href='/Contact'>
                    <a className="flex border-[1px] border-slate-500 px-[2.5rem] xl:text-[1rem] py-2 rounded-full tracking-widest hover:text-white hover:bg-purple-800 hover:border-transparent transition-all duration-500">
                        Contact Me
                    </a>
                </Link>
            </div>
